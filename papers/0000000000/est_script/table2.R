...
df <- read_dta("dataset/OST3datatoshare_temp.dta") %>%
  subset(., year>1979 & emerging == 1 & samp1 == 1 & samp2 == 1) %>%
  mutate(unit = ifscode)

# DEFINE PARAMETERS
dep <- 'lnresgdp'
fe_vars <- c('unit')
controls <- c('lpop', 'newkopen2', 'peg', 'softpeg', 'lnm2gdp', 'lntradegdp', 'evol', 'lngdppercap', 'lnstdtoutstandingdp')
free <- c("")
...
formula_df <- c('lnresgdp~lpop+lntradegdp+evol+lngdppercap',
                'lnresgdp~newkopen2+peg+softpeg+lnm2gdp+lntradegdp+lpop+evol+lngdppercap',
                'lnresgdp~newkopen2+peg+softpeg+lnm2gdp+lntradegdp',
                'lnresgdp~newkopen2+peg+softpeg+lnm2gdp',
                'lnresgdp~newkopen2+peg+softpeg+lnm2gdp+lntradegdp+lpop+evol+lngdppercap+lnstdtoutstandingdp',
                'lnresgdp~newkopen2+peg+softpeg+lnm2gdp+lntradegdp+lnstdtoutstandingdp') %>% 
