...
df <- read_dta("dataset/OST3datatoshare_temp.dta") %>%
  subset(., year>1979 & samp1 == 1 & samp2 == 1) %>%
  mutate(unit = ifscode)

# DEFINE PARAMETERS
dep <- 'lnresgdp'
fe_vars <- c('unit')
controls <- c('lpop', 'lntradegdp', 'lngdppercap', 'newkopen2', 'evol', 'peg', 'softpeg', 'lnm2gdp', 'advanced')
free <- c("")
maxregN <- length(controls)
tablename <- 'table1'

# LABEL VARIABLES
labels <- cbind(varname = c(dep, controls), label = c(dep, controls)) %>%
...
# PAPER ESTIMATION
# Create Formulas
formula_df <- c('lnresgdp~lpop+lntradegdp+evol+lngdppercap',
                'lnresgdp~newkopen2+lpop+lntradegdp+evol+lngdppercap',
                'lnresgdp~peg+softpeg+lpop+lntradegdp+evol+lngdppercap',
                'lnresgdp~lnm2gdp+lpop+lntradegdp+evol+lngdppercap',
                'lnresgdp~newkopen2+peg+softpeg+lnm2gdp+lntradegdp+advanced+lpop+evol+lngdppercap',
                'lnresgdp~newkopen2+peg+softpeg+lnm2gdp+lntradegdp+advanced',
                'lnresgdp~newkopen2+peg+softpeg+lnm2gdp+advanced') %>%
