...
df <- read_dta("dataset/OST3datatoshare_temp.dta") %>%
  subset(., year > 1979 & year < 1990) %>%
  mutate(unit = ifscode)

# DEFINE PARAMETERS
dep <- 'lnresgdp'
fe_vars <- c('unit')
controls <- c('newkopen2', 'peg', 'softpeg', 'lnm2gdp', 'lntradegdp', 'advanced')
free <- c("")
maxregN <- length(controls)
tablename <- 'table5_col4'
...
# PAPER ESTIMATION
# Create Formulas
formula_df <- c('lnresgdp~newkopen2+peg+softpeg+lnm2gdp+lntradegdp+advanced') %>% 
