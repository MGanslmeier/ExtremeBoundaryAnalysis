...
df <- read_dta("dataset/OST3datatoshare_temp.dta") %>%
  subset(., year > 1979  & samp1 == 1 & samp2 == 1 & emerging == 1) %>%
  mutate(unit = ifscode)

# DEFINE PARAMETERS
dep <- 'lnresgdp'
fe_vars <- c('unit')
controls <- c('newkopen2', 'peg', 'softpeg', 'lnm2gdp', 'lnmoneygdp', 'lntradegdp')
free <- c("")
maxregN <- length(controls)
tablename <- 'table6_col4'
...
# PAPER ESTIMATION
# Create Formulas
formula_df <- c('lnresgdp~newkopen2+peg+softpeg+lnm2gdp+lnmoneygdp+lntradegdp') %>%
